...
export const fetchItensRestaurantes: AsyncThunk<
  Restaurante[],
  void,
  { dispatch: AppDispatch }
> = createAsyncThunk<Restaurante[]>('itens/fetcheItens', async () => {
...
const itensSlice = createSlice({
  ...
  reducers: {
    escolherRestaurante(state, action: PayloadAction<number>) {
      state.itens = state.itens.filter(
        (restaurante) => restaurante.id === action.payload
      )
      console.log(
        'Restaturante Selecionado: ',
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchItensRestaurantes.fulfilled,
        (state, action: PayloadAction<Restaurante[]>) => {
